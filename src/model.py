...
class TA_GRU(tf.keras.Model):
    def __init__(self):
        ...
        self.attention_size = (self.topic_embedding_size + self.word_embed_size //
                               4) // 64 * 64 + (self.topic_embedding_size + self.word_embed_size//4) % 64
        ...
        self.rnn_hidden_size = (self.word_embed_size +
                                self.topic_embedding_size) // 4
        ...
        self.rnn_tweet = tf.keras.layers.LSTM(self.rnn_tweet_hidden_size, input_shape=[
                                              self.word_embed_size], return_sequences=True)
        ...
        self.A_alpha = tf.Variable(tf.random.normal(
            [self.fix_sentence_length], stddev=0.1))
        self.B_alpha = tf.Variable(tf.random.normal([1], stddev=0.1))

        self.A_tweets = tf.Variable(tf.random.normal(
            [(self.neighbor_tweets + 1)], stddev=0.1))
        self.B_tweets = tf.Variable(tf.random.normal([1], stddev=0.1))

        self.rnn = tf.keras.layers.GRU(self.rnn_hidden_size, input_shape=[(
            self.word_embed_size+self.topic_embedding_size)], return_sequences=True)
        self.linear = tf.keras.layers.Dense(
            self.class_count, input_shape=[self.rnn_hidden_size])
    ...
    def call(self, inputs):
        ...
        '''
        (batch_size, user_tweet_count, neighbor_tweet_count_add_one,
        ...
        var_only_wordembed_dim = tf.reshape(var_only_wordembed_dim, [
                                            batch_size, user_tweet_count, neighbor_tweet_count_add_one, self.fix_sentence_length, self.word_embed_size])
        var_only_wordembed_dim = tf.reshape(
            var_only_wordembed_dim, [-1, self.fix_sentence_length, self.word_embed_size])
        var_only_wordembed_dim_permuted = tf.transpose(
            var_only_wordembed_dim, perm=[1, 0, 2])
        ...
        var_rnn_tweet_output = tf.transpose(
            var_rnn_tweet_output, perm=[1, 0, 2])
        var_twitter_embedded = tf.math.reduce_mean(
            var_rnn_tweet_output, axis=1)  # default squeezed
        var_twitter_embedded = self.linear_tweet(var_twitter_embedded)
        ...
        var_only_wordembed_dim_permuted = tf.transpose(
            var_only_wordembed_dim, perm=[1, 0, 2])  # transpose (permute in torch)

        # var_rnn_tweet_output, (var_rnn_tweet_output_h, var_rnn_tweet_output_c) = self.rnn_tweet( var_only_wordembed_dim_permuted)
        var_rnn_tweet_output = self.rnn_tweet(var_only_wordembed_dim_permuted)

        # var_rnn_tweet_output = var_rnn_tweet_output.permute(1, 0, 2)
        var_rnn_tweet_output = tf.transpose(var_rnn_tweet_output, perm=[
                                            1, 0, 2])  # transpose in tensorflow
        ...
        var_twitter_embedded = self.linear_tweet(var_twitter_embedded)
        # var_twitter_embedded = var_twitter_embedded.view(batch_size, self.user_self_tweets, neighbor_tweet_count_add_one, self.word_embed_size )
        var_twitter_embedded = tf.reshape(var_twitter_embedded, [
                                          batch_size, self.user_self_tweets, neighbor_tweet_count_add_one, self.word_embed_size])
        ...
        var_twitter_and_topic_embedded = tf.concat(
            (var_twitter_embedded, var_only_topic_embedding), axis=3)
        # var_twitter_and_topic_embedded = var_twitter_and_topic_embedded.mul( 1.0/ ( self.constant_tweet_weight_a + 1) )
        var_twitter_and_topic_embedded = var_twitter_and_topic_embedded * \
            (1.0 / (self.constant_tweet_weight_a + 1))

        # ==========The original for tweet attention

        #var_twitter_and_topic_embedded = var_twitter_and_topic_embedded.permute( 0, 1, 3, 2 )
        var_twitter_and_topic_embedded = tf.transpose(
            var_twitter_and_topic_embedded, perm=[0, 1, 3, 2])
        ...
        var_user_tweet_context = tf.linalg.matvec(tf.reshape(var_twitter_and_topic_embedded, [-1, self.neighbor_tweets + 1]), self.A_tweets) + tf.reshape(
            tf.broadcast_to(self.B_tweets, [batch_size, user_tweet_count, self.word_embed_size + self.topic_embedding_size]), [-1])
        ...
        var_user = tf.reshape(var_user_tweet_context, [
                              batch_size, user_tweet_count, self.word_embed_size + self.topic_embedding_size])
        ...
        # var_seq_last = tf.gather(
        #     inputs, axis=1, indices=np.asarray(self.ids_seq_last, dtype=np.int32))
